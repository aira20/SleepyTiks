...
export const data = new SlashCommandBuilder()
  .setName('mm-dispute')
  .setDescription('Open a dispute on this transaction')
  .addStringOption(o => o.setName('reason').setDescription('Reason for dispute').setRequired(true));
...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  await mm.openDispute(ticket.transactionId, interaction.user.id, interaction.user.tag, reason, interaction);
}
